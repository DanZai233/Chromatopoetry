  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 md:p-8 animate-fade-in">
      ...
      <div className="relative w-full h-full max-w-6xl bg-white/90 rounded-3xl shadow-2xl flex flex-col overflow-hidden border border-white/40 ring-1 ring-black/5">
        ...
        {/* Toolbar */}
        <div className="flex flex-wrap items-center justify-between px-6 py-4 border-b border-gray-200/50 bg-white/50 backdrop-blur-sm gap-4">
          <div className="flex items-center gap-4">
             ...
             {/* Palette Name */}
             <span className="text-sm font-medium text-gray-500 font-mono hidden md:block">
             ...
             <div className="relative group">
                <div className="absolute inset-y-0 left-0 pl-2.5 flex items-center pointer-events-none">
                  <Layout className="h-3.5 w-3.5 text-gray-400" />
                </div>
                <select 
                  value={currentStyle}
                  onChange={(e) => onStyleChange(e.target.value as PreviewStyle)}
                  disabled={isLoading}
                  className="appearance-none bg-white border border-gray-200 hover:border-indigo-300 rounded-lg pl-8 pr-8 py-1.5 text-xs font-medium focus:outline-none focus:ring-2 focus:ring-indigo-500/20 cursor-pointer transition-all duration-200 shadow-sm text-gray-700 min-w-[140px]"
                >
